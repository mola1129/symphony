...
- When `codex.turn_sandbox_policy` is set explicitly, Symphony passes the map through to Codex
  ...
  Symphony validation.
- Workflows that require GitHub or other network services from Codex turns, such as `git push` or
  `gh pr create`, must set `codex.turn_sandbox_policy.networkAccess` to `true`.
